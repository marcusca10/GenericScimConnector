...
namespace Microsoft.AzureAD.Provisioning.ScimReference.Api.Controllers
{
    ...
    public class GroupsController : ControllerBase
    {
        ...
        private readonly string[] alwaysRetuned = ControllerConstants.AlwaysRetunedAttributes;
        ...
        public async Task<ActionResult<Core2Group>> Post(Core2Group item)
        {
            ...
            Resource group = await this._provider.GetByName(item.DisplayName).ConfigureAwait(false);
            if (group != null)
            {
        ...
        public async Task<IActionResult> Delete(string id)
        {
            try
            {
                Resource group = await this._provider.GetById(id).ConfigureAwait(false);
                if (group == null)
                {
                    ErrorResponse notFoundError = new ErrorResponse(string.Format(CultureInfo.InvariantCulture, ErrorDetail.NotFound, id), ErrorDetail.Status404);
                    return this.NotFound(notFoundError);
